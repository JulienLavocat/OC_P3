...
interface IntermediateMenu { name: string; deserts: MenuItem[]; starters: MenuItem[]; dishes: MenuItem[] }
interface MenuParts { name: string; items: MenuItem[] }
interface Menu { name: string; parts: MenuParts[] }
...
function parseMenu(tokens: string[]): Menu {
    ...
    const parseMenuItem = (token: string, container: MenuItem[], nextState: MenuParserState) => {
    ...
    const nextState = (token: string) => {
        if (!["Entrées", "Plats", "Desserts"].includes(token))
            return false;
...
function renderMenues(menues: Menu[]) {
    const namesToFiles: Record<string, { file: string, thumbnail: string }> = {
        "La palette du goût": { file: "la-palette-du-gout.html", thumbnail: "images/restaurants/jay-wennington-N_Y88TWmGwA-unsplash.jpg" },
        "Le délice des sens": { file: "le-delice-des-sens.html", thumbnail: "images/restaurants/jay-wennington-N_Y88TWmGwA-unsplash.jpg" },
        "La note enchantée": { file: "la-note-enchantee.html", thumbnail: "images/restaurants/jay-wennington-N_Y88TWmGwA-unsplash.jpg" },
        "À la française": { file: "a-la-francaise.html", thumbnail: "images/restaurants/jay-wennington-N_Y88TWmGwA-unsplash.jpg" }
    }
